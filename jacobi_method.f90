program jacobi_method
    ...
    real(real64) :: x(n), x_new(n)
    real(real64) :: residual_vector(n)
    real(real64) :: sum_value, error, residual
    ...
    open(newunit=output_unit, file='jacobi_convergence.dat', status='replace', action='write')

    write(output_unit,'(a)') '# iteration error residual x1 x2 x3'
    ...
    do iteration = 1, maximum_iterations
        ...
        residual_vector = b - matmul(A, x_new)
        residual = maxval(abs(residual_vector))

        write(output_unit,'(i8,1x,5(es20.10,1x))') iteration, error, residual, x_new(1), x_new(2), x_new(3)
    ...
    if (converged) then
        write(*,'(a,i0,a)') &
            'Jacobi method converged in ', iteration, ' iterations.'
    else
        write(*,'(a)') &
            'Jacobi method did not converge within the limit.'
    end if
